...
func paintSquareSlow(i, k int, src []int, img *image.Paletted) { // most expensive function
...
func paintSquareFastest(i, k int, src []int, img *image.Paletted) { // more optimized version
	ci := uint8(src[i])
	is, ks := i*scale, k*scale
	px := (ks-img.Rect.Min.Y)*img.Stride + (is-img.Rect.Min.X)*1
